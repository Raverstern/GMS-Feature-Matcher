...
inline Mat DrawInlier(Mat &src1, Mat &src2, vector<KeyPoint> &kpt1, vector<KeyPoint> &kpt2, vector<DMatch> &inlier, int type) {
    ...
    const int num_show = (100 < inlier.size() / 10) ? 100 : inlier.size() / 10;
    cv::RNG rng;
    Mat random_idx = Mat::zeros(1, num_show, CV_32SC1);
    rng.fill(random_idx, RNG::UNIFORM, 0, inlier.size());

    if (type == 1)
    {
        for (int c = 0; c < random_idx.cols; c++)
        {
            int idx = random_idx.at<int>(0, c);
            Point2f left = kpt1[inlier[idx].queryIdx].pt;
            Point2f right = (kpt2[inlier[idx].trainIdx].pt + Point2f((float)src1.cols, 0.f));
